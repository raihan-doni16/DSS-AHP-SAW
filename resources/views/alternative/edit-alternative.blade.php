    ...
    <div class="px-6 py-4">
        ...
        <form action="{{route('alternative.update')}}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="nama_karyawan" name="nama_karyawan" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" value="{{$alter->nama_karyawan}}">
            </div>
            <div class="mb-4">
                ...
                <select id="divisi" name="divisi" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm mb-10">
                    <option value="IT" {{ $alter->divisi == 'IT' ? 'selected' : '' }}>IT</option>
                    <option value="HR"{{ $alter->divisi == 'HR' ? 'selected' : '' }}>HR</option>
                    <option value="Finance"{{ $alter->divisi == 'Finance' ? 'selected' : '' }}>Finance</option>
                    <option value="Operations"{{ $alter->divisi == 'Operations' ? 'selected' : '' }}>Operations</option>
                </select>
                <div class="mb-4">
